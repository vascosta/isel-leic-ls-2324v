...
class PlayerApiTests {
    ...
    private val services = PlayerServices(dataMem)
    private val authHandler = AuthHandler(services)
    private val api = PlayerApi(services, authHandler)

    private val playerToken = createAndGetPlayerToken(api).token
    ...
    fun `try creating a player with invalid parameters and then returns a response with status 400`() {
        // Arrange
        val requestWithInvalidName = Request(Method.POST, "/player")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val requestWithInvalidEmail = Request(Method.POST, "/player")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val requestWithInvalidPassword = Request(Method.POST, "/player")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val requestWithInvalidInput = Request(Method.POST, "/player")
            .header("content-type", "application/json")
            .body("{}")
    ...
    fun `login a player by name and password and then returns a response with status code 200`() {
        // Arrange
        val createRequest = Request(Method.POST, "/player")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val loginRequest = Request(Method.PATCH, "/login")
            .header("content-type", "application/json")
            .body(
                """
                    {
    ...
    fun `try login a player that does not exist by name and password and then returns a response with status code 404`() {
        // Arrange
        val request = Request(Method.PATCH, "/login")
            .body(
                """
                    {
                        "name": "Manuel",
    ...
    fun `try login a player with invalid parameters and then returns a response with status code 400`() {
        // Arrange
        val requestWithInvalidName = Request(Method.PATCH, "/login")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val requestWithInvalidPassword = Request(Method.PATCH, "/login")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val requestWithInvalidInput = Request(Method.PATCH, "/login")
            .header("content-type", "application/json")
            .body("{}")
    ...
    fun `logout a player by token and then returns a response with status code 200`() {
        ...
        val logoutRequestWithBearer = Request(Method.PATCH, "/logout")
    ...
    fun `try logout a player that does not exist by token and then returns a response with status code 401`() {
        // Arrange
        val request = Request(Method.PATCH, "/logout")
    ...
    fun `gets a player home by token and then returns a response with status code 200`() {
        // Arrange
        val requestCreate = Request(Method.POST, "/player")
            .header("content-type", "application/json")
            .body(
                """
                    {
        ...
        val requestGetPlayerHome = Request(Method.GET, "/home/${playerInfo.token}")
    ...
    fun `try getting a player home that does not exist by token and then returns a response with status code 404`() {
        // Arrange
        val request = Request(Method.GET, "/home/1234567890")
    ...
    fun `gets a player info by id and then returns a response with status code 200`() {
        ...
        val requestWithBearer = Request(Method.GET, "/player/$playerId")
        ...
        assertEquals("Joaquim", bodyWithBearer.name)
        assertEquals("joaquim@email.com", bodyWithBearer.email)
    }
    ...
    fun `try getting a player info with no Authorization by id and then returns a response with status code 401`() {
        ...
        val request = Request(Method.GET, "/player/$playerId")
    ...
    fun `try getting a player info with an invalid id and then returns a response with status code 400`() {
        // Arrange
        val requestWithInvalidId = Request(Method.GET, "player/-1")
            .header("Authorization", "Bearer $playerToken")
        val requestWithInvalidId2 = Request(Method.GET, "player/manuel")
            .header("Authorization", "Bearer $playerToken")
    ...
    fun `try getting a player info that does not exist by id and then returns a response with status code 404`() {
        // Arrange
        val request = Request(Method.GET, "/player/1904")
            .header("Authorization", "Bearer $playerToken")
    ...
    fun `searches for players info by name and then returns a response with status 200`() {
        ...
        val request = Request(Method.GET, "/players/search?name=joaquim")
    ...
    fun `try searches for players info with an invalid name and then returns a response with status code 400`() {
        // Arrange
        val requestWithInvalidName = Request(Method.GET, "/players/search?name=joaquimdasilvapereiraalmeida")
    ...
    fun `try searching for players info that does not exist by name and then returns a response with status code 200`() {
        // Arrange
        val request = Request(Method.GET, "/players/search?name=manuel")
